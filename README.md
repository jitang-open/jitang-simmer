...
- **🤖 AI Token Statistics**：请求级统计 Codex、ZCode 和 DeepSeek Harness；展示输入、输出、缓存读取/写入、推理、总 Token、趋势、年度 P95 点阵、来源/模型排行；来源/provider/model 使用统一自定义下拉，面板内每日/每周/累计与顶栏双向同步；已经上传的历史不随本地日志、工具或扫描路径消失，累计按 SQLite 全部历史计算
...
- **📅 三种时间粒度**：每日（24 小时分布）/ 每周（近 7 天）/ 累计（全部历史；趋势图仅展示最近 12 个月），顶栏与 Token 面板内按钮双向同步；总时长和 Token 趋势默认收起，可按需展开
