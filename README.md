...
- **🌡️ 硬件监控**：基于 LibreHardwareMonitor 0.9.6 每分钟采集 CPU / GPU 占用、内存 / 显存占用、CPU / GPU 温度、可用功耗和硬盘占用；逐项缺失显示为不可用，不阻塞软件时长或 Token 统计。网页硬件面板每 10 分钟主动刷新，日视图按 144 个十分钟桶绘制，X 轴仍按 3 小时稀疏标注
- **📋 白名单**：只统计白名单内软件，开关即时生效、全站图表实时重算；添加软件时可搜索、滚动选择已观测进程或手动输入，软件类型使用统一下拉，清空必须二次确认；配置持久化到后端 SQLite，跨浏览器及 `localhost` / `127.0.0.1` 共享
- **💻 多设备同步**：多台采集器使用各自稳定 `deviceId` 向同一服务器定时同步；即使暂无新数据也会上报心跳。同步详情默认折叠，展开后显示在线/离线/暂停、最后同步、软件/Token/硬件通道与服务器记录数；默认跨设备聚合，也可单选设备查看
- **🗓️ 日期下钻**：点击软件或 Token 年度点阵格即可查看当天；两个卡片都使用与设备选择器一致的暗色下拉日历，可独立选择日、自然周、自然月或直接在统一日历内点选任意起止日期，Token 另有累计全部历史
...
1. 只运行一个中心 `server/`，本地测试可用 `http://服务器地址:8788`，远程设备建议使用文档中的 Caddy HTTPS 部署。
2. 每台电脑安装同一采集器，在托盘设置中填写相同的服务器地址和 Bearer Token；每台机器首次启动会生成不同的 `deviceId`，不要复制另一台机器的 `%APPDATA%\SimmerCollector\config.json`。
...
- [x] 多设备定时同步：空队列心跳、在线/离线状态、分通道时间、服务器数据量、全部设备聚合和单设备查看 ✅ v0.10.0
